...
import {
    ...
    productEdit,
    productList
} from './helpers/product-router-helper';
...
describe('PUT /products/:productID/edit - Product Edit Endpoint', () => {
    let authLoggedResponse: request.Response;
    ...
    it('Should be able to edit product', async () => {
        const productEdited = await productEdit(
            authLoggedResponse.body.data.Authorization,
            productListedResponse.body.data.Products[0]._id,
            { name: 'Indomie Rebus', code: 'PRD01' }
        );
        expect(productEdited.status).toEqual(200);
    });
});
